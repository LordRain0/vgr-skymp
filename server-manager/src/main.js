...
// A service may still be installed under an older name (the backend runs as
// SkyrpBackend on this box), so resolve which installed name to target:
...
// Best-effort LiveKit (voice media server) start alongside the game server; boxes without the service skip silently.
async function startLiveKitAlongside() {
  const status = await nssm('status', config.liveKitService)
  if (!/^SERVICE_/.test(status) || status === 'SERVICE_RUNNING') return ''
  await nssm('start', config.liveKitService)
  const r = await awaitStatus(config.liveKitService, 'SERVICE_RUNNING')
...
function chatLogDir() {
  return process.env.VGR_LOG_DIR || readServerSettings().logDir || 'C:\\logs'
}
...
async function serviceLogFiles(svc) {
  ...
  if (svc.key === 'game') {
    for (const f of ['chat.log', 'admin.log', 'pvp.log', 'trading.log']) {
...
const CONSOLE_HELP = [
  ...
  '  gamemode status|sync           compare / merge repo vgr-gamemode onto the server',
...
async function tryLocalCommand(cmd) {
  ...
  if (verb === 'gamemode') {
    if (arg === 'status') {
      const st = builder().gamemodeStatus()
      if (!st.ok) { consoleOut('gamemode status failed: ' + st.error); return { ok: true } }
      consoleOut(formatGamemodeStatus(st))
      return { ok: true }
    }
    if (arg === 'sync') {
      if (buildBusy) { consoleOut('a build is already running - wait for it to finish'); return { ok: true } }
      consoleOut('syncing repo gamemode onto the server…')
      runBuild('gamemode-sync').then(r => consoleOut(r.ok ? 'gamemode sync complete - restart the Game service to apply' : `gamemode sync failed: ${r.error || 'see log'}`))
      return { ok: true }
    }
    consoleOut('usage: gamemode <status|sync>')
    return { ok: true }
  }
  return null
}

function formatGamemodeStatus(st) {
  const lines = [`repo:   ${st.repoRoot}`, `server: ${st.srvRoot}`]
  lines.push(`in sync: ${st.same.length} file(s)`)
  if (st.differs.length) lines.push(`differs (sync would update): ${st.differs.join(', ')}`)
  if (st.repoOnly.length) lines.push(`repo-only (sync would add): ${st.repoOnly.join(', ')}`)
  if (st.serverOnly.length) lines.push(`server-only (kept, no repo source): ${st.serverOnly.join(', ')}`)
  if (!st.differs.length && !st.repoOnly.length) lines.push('server matches the repo.')
  return lines.join('\n')
}
...
// Launcher version lives in three places: skymp5-launcher/package.json (what a
// build stamps into the installer), LAUNCHER_LATEST_VERSION in the backend .env
// (what /api/version serves; snapshotted at backend start), and the
// FALLBACK_LATEST_VERSION const in routes/version.js. Write all three together.
...
ipcMain.handle('launcher:getVersion', () => {
  const served = readEnvValue(config.paths.backendEnv, 'LAUNCHER_LATEST_VERSION')
  let pkg = ''
  try { pkg = JSON.parse(fs.readFileSync(config.paths.launcherPkg, 'utf8')).version || '' } catch {}
  const version = served || pkg
  const mismatch = served && pkg && served !== pkg
    ? `served ${served} but skymp5-launcher/package.json says ${pkg}` : ''
  return { version, served, pkg, mismatch }
})
...
ipcMain.handle('launcher:setVersion', (_e, version) => {
  ...
  try {
    ...
    setEnvVar(config.paths.backendEnv, 'LAUNCHER_LATEST_VERSION', version)
    setRouteVersion(config.paths.versionRoute, 'FALLBACK_LATEST_VERSION', version)
    return { ok: true, note: 'Wrote package.json + .env + version.js. Restart the Backend service to serve it.' }
  } catch (err) { return { ok: false, error: err.message } }
})

// Client files version (read-only): stamped by the client packaging pipeline.
ipcMain.handle('files:getVersion', () => {
  try {
    const j = JSON.parse(fs.readFileSync(config.paths.filesVersion, 'utf8'))
    return { ok: true, version: j.version || '', builtAt: j.builtAt || '', fileCount: j.fileCount, zipSize: j.zipSize }
  } catch (err) { return { ok: false, error: err.message } }
})
...
// Optional flat-file whitelist badge; this backend normally whitelists via a
// Discord role, so an absent data/whitelist.json just means no badges.
...
ipcMain.handle('players:update', async (_e, profileId, patch) => {
  try {
    const clean = {}
    for (const k of ['username', 'displayName', 'notes']) {
      if (patch && patch[k] !== undefined) clean[k] = String(patch[k] ?? '')
    }
    const updated = await backendModule('players').updateByProfileId(Number(profileId), clean)
...
// ── News tab: launcher news entries + image picker ─────────────────────────────

ipcMain.handle('news:list', () => {
  try { return { ok: true, items: news.readItems(), path: config.paths.newsFile, today: news.todayLabel() } }
  catch (err) { return { ok: false, error: err.message } }
})

ipcMain.handle('news:save', (_e, items) => {
  try { return { ok: true, items: news.writeItems(items) } }
  catch (err) { return { ok: false, error: err.message } }
})

ipcMain.handle('news:images', () => {
  try { return { ok: true, ...news.listImages() } }
  catch (err) { return { ok: false, error: err.message } }
})

// Copy a .gif (or other image) from anywhere on disk into the served folder.
ipcMain.handle('news:importImage', async () => {
  const r = await dialog.showOpenDialog(win, {
    title: 'Import a news image',
    filters: [
  ...
  try { return { ok: true, image: news.importImage(r.filePaths[0]) } }
  catch (err) { return { ok: false, error: err.message } }
})
...
// Parse a .env-style file into { values } preserving unknown lines on write.
...
ipcMain.handle('modlist:read', () => {
  ...
  if (!modlist) return { ok: false, error: `No modlist.txt under ${profileDir}. Check VGR_MO2_ROOT / VGR_MO2_PROFILE.` }
