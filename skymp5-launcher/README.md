# Vengeful Realms Launcher

Desktop launcher for the Vengeful Realms SkyMP server. Handles Discord authentication, client file installation, mod management via Mod Organizer 2, and launching Skyrim through SKSE.
UI styled after the original VengefulRealms-Launcher: https://github.com/datboyonsum/VengefulRealms-Launcher
Original launcher core by the SkyMP team: https://github.com/F02K/SkyMP-Launcher

## Instructions

1) Open the Server Manager (server-manager) and go to the Launcher tab
2) Set the version if needed, then click "Rebuild"
3) Collect the installer from build\launcher\VengefulRealmsLauncher.exe (already named for distribution)
4) Route <your-api-domain>/download/VengefulRealmsLauncher.exe to the installer as needed
...
src/
  ...
  config.js        API_URL from env (defaults to https://api.vengefulrealms.com,
                   the production backend; use .env for local dev)
  ...
  renderer/
    index.html     UI shell: sidebar nav, Play/Realms/Mod Pack/Notices sections, modals
    renderer.js    Event listeners, API calls, settings, news/modlist rendering
    styles.css     Vengeful Realms theme: bronze dividers, dark panels, red accents
assets/            App icon (icon.ico), VGR logo.png, icons/ (fantasy SVG set),
                   background.gif (Play-section ambient backdrop), controlmap.txt
```
...
| `baseDirPath` | string | Vengeful Realms base dir: MO2 root, with the game copy at `<base>\skyrim` |
