...
::       api. and dashboard.vengefulrealms.com (auto-renewing)
::    4. Switches nginx to the full HTTPS proxy config
::       (api -> localhost:4000, dashboard -> localhost:4002)
::    5. Registers nginx as the vengefulrealmsNginx Windows service
...
set "NSSM_DIR=C:\tools\nssm"
set "EMAIL=adams.media@outlook.com"
set "DOMAINS=api.vengefulrealms.com,dashboard.vengefulrealms.com"
set "CERT_NAME=api.vengefulrealms.com"
set "SERVICE=vengefulrealmsNginx"
...
:: ---- Legacy service cleanup (runs on every invocation) ----
...
:: --installation script makes the renewal task reload nginx so new pems go live
:: (copied to C:\nginx first so renewals keep working if the repo moves)
copy /y "%REPO_DIR%\deploy\nginx\reload-nginx.bat" "%NGINX_DIR%\reload-nginx.bat" >nul
"%WACS_DIR%\wacs.exe" --source manual --host "%DOMAINS%" --validation filesystem --webroot "%NGINX_DIR%\html" --store pemfiles --pemfilespath "%NGINX_DIR%\certs" --installation script --script "%NGINX_DIR%\reload-nginx.bat" --accepttos --emailaddress %EMAIL%
...
echo   curl https://api.vengefulrealms.com/api/status
